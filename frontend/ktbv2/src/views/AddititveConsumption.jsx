...
function AdditiveConsumption() {
  ...
  const [currentPage, setCurrentPage] = useState(1);
            ...
            if (searchField === "all") {
                filtered = filtered.filter((item) => {
                    return (
                        ...
                        (item.additive_name || "").toString().toLowerCase().includes(lowerSearch) ||
    ...
    const handleDownloadExcel = async () => {
        try {
            const response = await axios.get('/excel/export/report/additive-cons/', {
                responseType: 'blob', // Important
            });
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'additive_consumption_report.xlsx');
