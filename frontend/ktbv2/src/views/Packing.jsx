...
<button
        ...
        <div>
        <CostMgtFilterComponent checkBtn={false} flag={2} onFilter={handleFilter} apiEndpoint={'/costmgt/packings/'} fieldOptions={fieldOptions} downloadUrl="/excel/export/packing/" fileName="Packing_export" statusField="approved" />
        </div>
...
<button onClick={approvePacking} className="bg-blue-500 text-white p-2 rounded col-span-3">Approve</button>
              ...
              <div className="mt-4 p-4 border-t border-gray-200 bg-gray-50 rounded">
                ...
                {selectedPacking?.notified_users_emails?.length > 0 ? (
                  <ul className="list-disc pl-5">
                    {selectedPacking.notified_users_emails.map((email, idx) => (
