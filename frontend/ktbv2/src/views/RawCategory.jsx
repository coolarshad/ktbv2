...
const RawCategory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/costmgt/raw-categories/');
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        await axios.delete(`/costmgt/raw-categories/${id}/`);
  ...
  const handleViewClick = async (id) => {
    try {
      const response = await axios.get(`/costmgt/raw-categories/${id}/`);
  ...
  const handleEdit = (id) => {
    navigate(`/raw-category-form/${id}`);  // Navigate to TradeForm with tradeId
  };
              ...
              {currentItems?.map((category) => (
                <tr key={category.id} className="border-b border-gray-200">
                  ...
                  <td className="py-2 px-4">
                    ...
                    {hasPermission(user, 'update_raw_material_category') && (
                      <button
                    ...
                    {hasPermission(user, 'delete_raw_material_category') && (
                      <button
        ...
        {selectedCategory && (
          <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center">
            <div className="bg-white w-1/2 p-4 rounded-lg shadow-lg overflow-auto">
              ...
              <h2 className="text-2xl mb-2 text-center">Raw Category Details</h2>
              <hr className='mb-2' />
              <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-sm">
                <tbody>
