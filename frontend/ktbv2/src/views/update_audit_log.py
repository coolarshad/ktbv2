...
const PAGE_OPTIONS = [
  ...
  { name: 'Trade Approval', keyword: '/trade-approval' },
  { name: 'Trade Approved', keyword: '/trade-approved' },
  { name: 'Pre-Sale Purchase', keyword: '/pre-sale-purchase' },
...
export default function AuditLog() {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      // Find the keyword for the selected page
      const pageObj = PAGE_OPTIONS.find(p => p.name === selectedPage);
      if (!pageObj || !pageObj.keyword) return;
      
      // Remove leading slash to make a flexible search keyword
      let searchKeyword = pageObj.keyword.startsWith('/') ? pageObj.keyword.substring(1) : pageObj.keyword;
      if (searchKeyword === '') {
         // for Dashboard we skip or search for a specific keyword
         searchKeyword = 'dashboard';
      }

      const url = `/accounts/logs/?search=${encodeURIComponent(searchKeyword)}`;
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-10 text-center text-sm text-gray-500">
                    {selectedPage ? `No audit logs found for ${selectedPage}.` : 'Please select a page to view audit logs.'}
                  </td>
                </tr>
              ) : (
                logs.map((log, index) => (
...
with open('/home/saiyad/ktbv2/ktbv2/frontend/ktbv2/src/views/AuditLog.jsx', 'w') as f:
    f.write(new_content)
